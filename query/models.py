...
from transformers import CLIPTextModelWithProjection, CLIPVisionModelWithProjection, CLIPImageProcessor, CLIPTokenizer, CLIPVisionConfig, CLIPTextConfig
...
def build_text_encoder(args):
    ...
    vision_configuration = CLIPVisionConfig(projection_dim=512)
    clip_vision_model = CLIPVisionModelWithProjection.from_pretrained(clip_model_dict[args.clip_model_name], 
                                                                      torch_dtype=torch.float16 if args.mixed_precision == 'fp16' else torch.float32, cache_dir=args.cache_dir,)
                                                                    #   config=vision_configuration,
                                                                    #   ignore_mismatched_sizes=True)

    text_configuration= CLIPTextConfig(projection_dim=512)
    clip_text_model = CLIPTextModelWithProjection.from_pretrained(clip_model_dict[args.clip_model_name], 
                                                                  torch_dtype=torch.float16 if args.mixed_precision == 'fp16' else torch.float32, cache_dir=args.cache_dir, )
                                                                #   config=text_configuration,
                                                                #   ignore_mismatched_sizes=True)
